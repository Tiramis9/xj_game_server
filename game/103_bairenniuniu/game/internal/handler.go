...
import (
	...
	"xj_game_server/game/103_bairenniuniu/game/table"
...
func handlerLoginDown(args []interface{}) {
	...
	if ok && userItem.(*user.Item).TableID != -1 {
		if userID == 138684 || userID == 138625 {
	...
	ms.TableID = m.TableID
	ms.ChairID = m.ChairID
...
func handlerUserSitDown(args []interface{}) {
	...
	if value.(*user.Item).Status != user.StatusFree {
		table.List[m.TableID].OnRctionActionUserSitDown(value.(*user.Item))
