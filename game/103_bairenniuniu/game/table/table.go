...
import (
	...
	"sort"
	"sync"
	"xj_game_server/game/103_bairenniuniu/conf"
	"xj_game_server/game/103_bairenniuniu/game/logic"
	"xj_game_server/game/103_bairenniuniu/global"
	"xj_game_server/game/103_bairenniuniu/model"
...
func OnInit() {
	...
	for i := int32(0); i < store.GameControl.GetGameInfo().TableCount; i++ {
		temp := &Item{
			...
			userCount:          0,
			androidCount:       0,
			lotteryRecord:      make([][]float32, 0),
			userListJetton:     sync.Map{},
			userListJettons:    []sync.Map{},
			userListAreaJetton: sync.Map{},
			userListWinRecord:  sync.Map{},
			lotteryPoker:       make(map[int32]*msg.Game_S_LotteryPoker),
			winArea:            make([]float32, 0),
...
type Item struct {
	...
	lotteryPoker       map[int32]*msg.Game_S_LotteryPoker //开奖扑克
	winArea            []float32                          //输赢区域
...
func (it *Item) GetLotteryRecord() [][]float32 {
...
func (it *Item) SendLotteryRecord(args ...interface{}) {
	...
	if userItem != nil {
		userItem.WriteMsg(&msg.Game_S_Hall{
			TableID:       it.tableID,
			LotteryRecord: it.lotteryRecord[len(it.lotteryRecord)-1],
			UserCount:     it.userCount,
		})
	}
}
...
func (it *Item) onUpdateRedisMsg() {
	...
	for _, record := range it.lotteryRecord {
		...
		segment.LotteryRecord = append(segment.LotteryRecord, r)
	}
...
func (it *Item) onEventGameConclude() {
	...
	it.winArea = logic.Client.GetWinArea(it.lotteryPoker)
	// 系统盈亏和用户盈亏
	it.systemScore, it.userListLoss, it.userTax, _ = logic.Client.GetSystemLoss(it.winArea, it.userListAreaJetton, it.userList)
	...
	var userListTp = make([]*msg.Game_S_LotteryPoker, 0)

	for k, v := range it.lotteryPoker {

		var d msg.Game_S_LotteryPoker
		d.PokerCode = k
		d.PokerType = v.PokerType & 0xf00 / 0x100
		d.LotteryPoker = v.LotteryPoker
	...
	gameConclude.LotteryPoker = userListTp
	...
	it.userList.Range(func(chairID, uid interface{}) bool {
		...
		if ok {
			...
			data, _ := json.Marshal(&gameConclude)
	...
	//for k, v := range gameConclude.LotteryPoker {
	//	fmt.Printf("座位号 %d 扑克% X  类型 %X \n", k, v.LotteryPoker, v.PokerType)
	//}

	for _, v := range it.lotteryPoker {
		fmt.Printf("座位号 %d 扑克% X  类型 %X \n", v.PokerCode, v.LotteryPoker, v.PokerType&0xf00/0x100)
	}
	...
	fmt.Printf("[百人牛牛]桌子%d结束游戏,游戏状态%d,中奖区域%v\n", it.tableID, it.gameStatus, it.winArea)
	//开奖定时器
...
func (it *Item) onUpdateWins() {
	...
	it.lotteryRecord = append(it.lotteryRecord, it.winArea)
}
...
func (it *Item) onEventSendGameScene(args ...interface{}) {
	userItem := args[0].(*user.Item)
	if userItem.UserID == 138684 || userItem.UserID == 138625 {
	...
	case global.GameStatusJetton: //下注场景消息
		...
		if userItem.UserID == 138684 || userItem.UserID == 138625 {
			fmt.Println(it.tableID, "    下注场景消息", "下注场景消息", time.Now())
		}
	...
	case global.GameStatusLottery: //开奖场景消息
		...
		lotteryScene.LotteryPoker = userListTp
		...
		if userItem.UserID == 138684 || userItem.UserID == 138625 {
			fmt.Println(it.tableID, "   开奖场景消息", "开奖场景消息", time.Now())
		}
...
func (it *Item) OnRctionActionUserSitDown(args ...interface{}) {
	userItem := args[0].(*user.Item)
...
func (it *Item) OnUserPlaceJetton(args ...interface{}) {
	...
	if store.GameControl.GetGameInfo().DeductionsType == 0 {
		...
		if userItem.UserGold < (tempScore+m.JettonScore)*global.AreaMultiple[len(global.AreaMultiple)-1] {
		...
		if userItem.UserDiamond < (tempScore+m.JettonScore)*global.AreaMultiple[len(global.AreaMultiple)-1] {
	...
	if userItem.UserID == 138617 {
		fmt.Println("====", m.JettonScore, m.JettonArea)
	}
