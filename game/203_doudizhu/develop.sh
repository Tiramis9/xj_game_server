...
do
echo ${host}
./upload.expect ../../bin/${PROJECT_NAME}_new ${USER_NAME} ${host} ${PASSWORD} /usr/local/server/game/ddz/ddz$1/bin
if [[ "$?" != 0 ]]; then
   exit 2
fi
done
...
do
echo ${host}
./restart.expect ${PROJECT_NAME} ${USER_NAME} ${host} ${PASSWORD} /usr/local/server/game/ddz/ddz$1/bin 203_ddz_$1
done
