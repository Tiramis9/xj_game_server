...
import (
	...
	"xj_game_server/game/105_benchibaoma/game/logic"
	"xj_game_server/game/105_benchibaoma/global"
	"xj_game_server/game/105_benchibaoma/model"
...
type Item struct {
	...
	lotteryPoker       int32                    //开奖号码 奔驰宝马 保时捷
	lotteryLocation    int32                    //开奖号码位置
...
func (it *Item) sendUserAreaLottery() {
	for {
		...
		it.cronTimer = time.NewTimer(1000 * time.Millisecond)
		select {
		case <-it.cronTimer.C:
			if it.gameStatus == global.GameStatusJetton {
				var areaJettions = make([]*msg.JettonInfoResp, 0)
				for i := 0; i < global.AreaCount; i++ {
					var tempArearJettion msg.JettonInfoResp
					...
					tempArearJettion = msg.JettonInfoResp{
						Area:  int32(i),
						Money: tempJetton,
					}
					areaJettions = append(areaJettions, &tempArearJettion)
				}
				it.infoList = areaJettions
				// 发送每个区域每个人的总下注
...
func (it *Item) onEventGameStart() {
	...
	it.cronTimer.Reset(100 * time.Millisecond)
	...
	fmt.Printf("[奔驰宝马]桌子%d开始游戏,游戏状态%d,游戏人数:%d\n", it.tableID, it.gameStatus, it.userCount)
...
func (it *Item) onEventGameConclude() {
	...
	it.lotteryLocation = rand.RandInterval(1, 4)
	...
	//var min float32
	//var count int32
	//fmt.Println("下标", it.lotteryLocation)
	//fmt.Printf("[奔驰宝马]==测试死循环==桌子%d结束游戏,随机值%d 数据库值:%f\n", it.tableID, randNumber, store.GameControl.GetUserWinRate())
	////开奖
	//for {
	//	// 系统盈亏和用户盈亏
	...
	//	// 获取赢的区域
	//	it.winArea = logic.Client.GetWinArea(it.lotteryPoker)
	//	// 系统盈亏和用户盈亏
	...
	for i := 0; i < 5; i++ {
		...
		// 发牌
		w.LotteryPoker = append(w.LotteryPoker, logic.Client.DispatchTableCard())
		// 获取赢的区域
		w.WinArea = logic.Client.GetWinArea(w.LotteryPoker[0])
		// 系统盈亏和用户盈亏
	...
	it.lotteryPoker = winAreas[0].LotteryPoker[0]
	...
	//gameConclude.LotteryLocation = it.lotteryLocation
	////float32(rand.RandInterval(45,60)) / 10
	//gameConclude.SlowDown = 6
	// 发自己和特殊的四人的记录
	...
	it.userList.Range(func(chairID, uid interface{}) bool {
		...
		if ok {
			...
			gameConclude.Index = (it.lotteryLocation * 8) - 8 + it.lotteryPoker
			gameConclude.Area = it.lotteryPoker
	...
	fmt.Printf("[奔驰宝马]桌子%d结束游戏,游戏状态%d\n,开奖号码% X\n,开奖列表%v\n,中奖区域%v\n", it.tableID, it.gameStatus, it.lotteryPoker, it.lotteryRecord, it.winArea)
	//清空上局数据
	it.systemScore = 0
	it.drawID = ""
	//开奖定时器
...
func (it *Item) onEventSendGameScene(args ...interface{}) {
	...
	case global.GameStatusJetton: //下注场景消息
		...
		var gameSAreajetton msg.Game_S_AreaJetton
		gameSAreajetton.InfoList = areaJettions
		jettonScene.AreaInfo = gameSAreajetton

		//玩家每个区域的下注数
		var userJettonList = make([]*msg.JettonInfoResp, 0)
		load, ok := it.userListAreaJetton.Load(userItem.ChairID)
		if ok {
			for k, v := range load.([global.AreaCount]float32) {
				userJetton := &msg.JettonInfoResp{
					Area:  int32(k),
					Money: v,
				}
		...
		var gameSAreajetton1 msg.Game_S_AreaJetton
		gameSAreajetton1.InfoList = userJettonList
		jettonScene.JettonInfo = gameSAreajetton1
		jettonScene.SurplusTime = float64(sceneTime)
		jettonScene.RecordID = it.roundOrder

		userItem.WriteMsg(&jettonScene)
	case global.GameStatusLottery: //开奖场景消息
		var jettonScene msg.Game_S_ConcludeScene
		var areaJettions = make([]*msg.JettonInfoResp, 0)
		for i := 0; i < global.AreaCount; i++ {
			var tempArearJettion msg.JettonInfoResp
			var tempJetton float32
		...
		var gameConclude msg.Game_S_GameConclude
		gameConclude.Index = it.lotteryLocation * it.lotteryPoker
		gameConclude.Area = it.lotteryPoker
		gameConclude.Money = it.userListLoss[userItem.ChairID]
		jettonScene.Result = gameConclude
		jettonScene.SurplusTime = float64(sceneTime)
		jettonScene.JettonInfo.InfoList = userJettonList

		jettonScene.AreaInfo.InfoList = areaJettions
		userItem.WriteMsg(&jettonScene)
	}
}
...
func (it *Item) OnActionUserSitDown(args ...interface{}) {
	...
	userItem.WriteMsg(&msg.Game_S_LoginSuccess{
		...
		History:       it.lotteryRecord[:lengh],
		ChipNum:       conf.GetServer().JettonList,
	})
	//it.lotteryRecord[len(it.lotteryRecord)-1:][0]
	//发送场景消息
...
func (it *Item) OnActionUserOffLine(args ...interface{}) {
	...
	//断线通知桌面上的人
	if it.isOnTable(userItem) {
		it.sendOtherUser(userItem.UserID, &msg.Game_S_StandUpNotify{
...
func (it *Item) OnUserPlaceJetton(args ...interface{}) {
	...
	//检测数据是否异常
	if m.Area < 0 || m.Area >= global.AreaCount || m.Money <= 0 {
	...
	if store.GameControl.GetGameInfo().DeductionsType == 0 {
		...
		if userItem.UserGold < tempScore+float32(m.Money) {
		...
		if userItem.UserDiamond < tempScore+float32(m.Money) {
	...
	if !userItem.IsRobot() {
		err := redis.GameClient.SetDiamond(userItem.UserID, userItem.UserDiamond-(tempScore+float32(m.Money)))
		if err != nil {
			userItem.WriteMsg(&msg.Game_S_ReqlyFail{
				ErrorCode: global.JettonError2,
				ErrorMsg:  "下注失败",
	...
	if score, ok := it.userListJetton.Load(userItem.ChairID); ok {
		temp := score.(float32)
		temp += float32(m.Money)
		it.userListJetton.Store(userItem.ChairID, temp)
	} else {
		it.userListJetton.Store(userItem.ChairID, m.Money)
	}
	...
	//	})
	//}
	var info msg.JettonInfoResp
	info.Area = m.Area
	info.Money = float32(m.Money)
	userItem.WriteMsg(&msg.Game_S_UserJetton{Info: info})
