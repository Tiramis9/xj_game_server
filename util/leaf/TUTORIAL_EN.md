...
* Multi-core support. Leaf utilize its modules and [leaf/go](https://util/leaf/tree/master/go) to make use of CPU resouces at maximum while avoiding varieties of side effects may be caused.
...
A game server implemented with Leaf may include many modules (e.g. [LeafServer](https://util/leafserver)) which all share below traits:

* Each module runs inside a separate goroutine
* Modules communicate with one another via a light weight RPC channel([leaf/chanrpc](https://util/leaf/tree/master/chanrpc))
...
[LeafServer](https://util/leafserver) is a game server developped with Leaf. Let's start with it.

Download the source code of LeafServer：

```
git clone https://util/leafserver
```
...
More references are at [leaf/go](https://util/leaf/blob/master/go)。
...
More references are at [leaf/timer](https://util/leaf/blob/master/timer)。
...
More references are at [leaf/log](https://util/leaf/blob/master/log).

### Leaf recordfile

Leaf recordfile is formatted in CSV([Example](https://util/leaf/blob/master/recordfile/test.txt)). recordfile is to manage the configuration for game. The usage of recordfile in LeafServer is quite simple:
...
Refer to [leaf/recordfile](https://util/leaf/blob/master/recordfile) for more details.

Learn more
----------

More references are at Wiki [https://util/leaf/wiki](https://util/leaf/wiki)
