...
func (p *Processor) Register(msg interface{}) string {
	...
	if msgType == nil || msgType.Kind() != reflect.Ptr {
		_ = log.Logger.Errorf("json message pointer required")
		golog.Fatalf("json message pointer required")
	}
	msgID := msgType.Elem().Name()
	if msgID == "" {
		_ = log.Logger.Errorf("unnamed json message")
		golog.Fatalf("unnamed json message")
	}
	if _, ok := p.msgInfo[msgID]; ok {
		_ = log.Logger.Errorf("message %v is already registered", msgID)
		golog.Fatalf("message %v is already registered", msgID)
	}
...
func (p *Processor) SetRouter(msg interface{}, msgRouter *chanrpc.Server) {
	...
	if !ok {
		_ = log.Logger.Errorf("message %v not registered", msgID)
		golog.Fatalf("message %v not registered", msgID)
	}
...
func (p *Processor) SetHandler(msg interface{}, msgHandler MsgHandler) {
	...
	if msgType == nil || msgType.Kind() != reflect.Ptr {
		_ = log.Logger.Errorf("json message pointer required")
		golog.Fatal("json message pointer required")
	}
	msgID := msgType.Elem().Name()
	i, ok := p.msgInfo[msgID]
	if !ok {
